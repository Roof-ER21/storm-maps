...
export const READ_TOOLS: ToolDef[] = [
  ...
  { name: 'aggregate_projects', description: "Job counts + TOTAL BOOKED VALUE grouped by a dimension. total_value spans ALL stages incl. pending + dead/cancelled — it is NOT completed revenue (it overstates). For completed revenue use the summary tools (carrier → get_carriers_summary.revenue). The carrier dimension is 'insurance'.", kind: 'read', roles: ['analytics', 'admin', 'exec'], method: 'GET', path: '/api/intel/projects-aggregate', params: obj({ group_by: { type: 'string', enum: ['insurance', 'zip', 'state', 'city', 'stage', 'sales_rep', 'lead_source', 'job_type'], description: "Dimension to group by — 'insurance' is the carrier." } }, ['group_by']) },
  ...
  { name: 'get_carriers_summary', description: 'Carrier list with per-carrier COMPLETED revenue (completed/finalized jobs; this is the canon "revenue") + approval rates. Use this and rank by its `revenue` field for "top carriers by revenue" / carrier completed revenue.', kind: 'read', roles: 'any', method: 'GET', path: '/api/intel/carriers-summary', params: noParams },
